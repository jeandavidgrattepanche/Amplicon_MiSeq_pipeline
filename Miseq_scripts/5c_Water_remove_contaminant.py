...
def test(seqfile, seqdict):
	...
	for line in open('outputs/chimeras/water.txt','r'):
#		outfinal.write(line)
		if re.search('# 1: ', line):
...
#				gp = gp + 1
	...
	if float(sim)/float(len) * 100 < float(65):# and sim/ident > 1.02: #len/tlength > 0.67 and sim/len > 0.75 and 
...
def main():
	...
	for Seq in SeqIO.parse(seqfile,'fasta'):
		...
		if int(readnumber) == int(readmax):
			refseq=open('SAR_db/ref_for_water.fasta','w+')
			refseq.write('>'+Seq.id+'\n'+str(Seq.seq)+'\n')
			refseq.close()
	...
	for Seq in SeqIO.parse(seqfile,'fasta'):
		if Seq.id not in seqlist:
			...
			outseq=open('outputs/chimeras/seqtemp.fas','w')
			outseq.write('>'+Seq.id+'\n'+str(Seq.seq)+'\n')
			outseq.close()
			cline = 'water -asequence=SAR_db/ref_for_water.fasta -bsequence=outputs/chimeras/seqtemp.fas -gapopen=10 -gapextend=0.5 -outfile=outputs/chimeras/water.txt'		
#			print(cline)
			os.system(cline)
