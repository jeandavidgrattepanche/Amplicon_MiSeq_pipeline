...
def countread(seqfile,BLASTtsv, treetaxo,otufile,samplelist): #,dataname):	
	...
	for record in open(BLASTtsv,'r'):
		BLASTresults = ('_').join(record.split('\t')[2].split('_')[2:9])+';'+record.split('\t')[3]+';'+record.split('\t')[4]+';'+record.split('\t')[5]
		BLAST[record.split('\t')[0]] = BLASTresults
		
	for element in open(treetaxo,'r'):
		print(element.split('\t')[0].replace('QUERY___','').split('_')[0].split('-')[0])
		Tass = ('_').join(element.split('\t')[3].split('\n')[0].split('_')[2:9])+';'+element.split('\t')[1]+';'+element.split('\t')[2]
		TA_tree[element.split('\t')[0].replace('QUERY___','').split('_')[0].split('-')[0]] = Tass
	outfile = open('outputs/OTUs_ingroup/OTUtable_ingroup.txt','w')
	outfile.write('OTU\tBtaxo_rank1\tBtaxo_rank2\tBtaxo_rank3\tBtaxo_rank4\tBgenus\tBsp\tBacc_number\tid%\tEvalue\tcov%\tTtaxo_rank1\tTtaxo_rank2\tTtaxo_rank3\tTtaxo_rank4\tTgenus\tTsp\tTacc_number\tnode\tBranch_L\toccurrence\treadnumber\t' + str(samplelist).replace("', '",'\t').replace("[",'').replace("]",'').replace("'","") + '\n') #add the heading row with samples name
	outfile.close()
