...
def countread(readmap, readtokeep, samplelist): 	
	folder = readmap.split('/')[0]
	if not os.path.exists(folder + '/OTUs_ingroup/'):
		os.makedirs(folder + '/OTUs_ingroup/')
	output = open(folder + '/OTUs_ingroup/' + readmap.split('/')[-1].split('.')[0] + '_subsampled.txt','w+')
